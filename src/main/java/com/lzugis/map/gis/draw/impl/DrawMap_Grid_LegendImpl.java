...
public class DrawMap_Grid_LegendImpl extends DrawMap {

	public DrawMap_Grid_LegendImpl() {
	}
	...
	private boolean drawImage(JSONArray lg, double[][] points,
		...
		try {
			...
			Date se_cg = new Date();
			System.err.println("插值 花费时间是 : "
					+ ((se_cg.getTime() - sd_cg.getTime()) / 1000) + " 秒");
			...
			if (gridPoints != null) {
				...
				if (gridlayer != null) {
					...
					Date sd_gl = new Date();
					System.err.println("网格 花费时间是: "
							+ ((sd_gl.getTime() - se_cg.getTime()) / 1000)
							+ " 秒");
				} else {
