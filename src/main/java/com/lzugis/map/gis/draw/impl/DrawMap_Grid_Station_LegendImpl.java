...
public class DrawMap_Grid_Station_LegendImpl extends DrawMap {

	public DrawMap_Grid_Station_LegendImpl() {
	}
		...
		try {
			...
			for (int i = 0; i < cityCodes.size(); i++) {
				...
				oldfd = drawImage(pd.getLegend(), pd.getPoints(),
						pd.getStations(), pd.getFc(), pd.getImgName(),
						pd.getTitle(), pd.getWidth(), pd.getHeight());
	...
	private boolean drawImage(JSONArray lg, double[][] points, JSONArray ss,
		...
		try {
			...
			try {
				// 离散点
				Layer pointStationLayer = getStationPointLayer(ss);
				if (pointStationLayer != null) {
					map.addLayer(pointStationLayer);
				} else {
					System.err.println("生成站点失败!");
					return false;
				}
			} catch (Exception e) {
				e.printStackTrace();
			...
			// 创建图例
			if (!creatLegend(lg, bi, mapW / 8 * 1, lengonflag)) {
	 ...
	 * @return
	 */
	private Layer getStationPointLayer(JSONArray ss) {
		Layer pointlayer = null;
		try {
			...
			if (ss != null && ss.size() > 0) {
				...
				for (int i = 0; i < ss.size(); i++) {
					...
					if (so.containsKey("x") && so.containsKey("y")
							&& so.containsKey("v") && so.containsKey("sName")) {
						Coordinate coord = new Coordinate(
								so.getDoubleValue("x"), so.getDoubleValue("y"));
						Point point = geometryFactory.createPoint(coord);
						// if (gridGeo != null && gridGeo.contains(point)) {
						map.put("the_geom", point);
						map.put("value", so.getDoubleValue("v") + "");
						map.put("sname", so.getString("sName"));
						values.add(map);
						// }
					}
				...
				SimpleFeatureCollection sc = FeaureUtil
						.creatSimpleFeatureByFeilds(
								"points",
								"crs:4326,the_geom:Point,value:String,sname:String",
								values);
				FeatureSource fs = FeaureUtil
						.creatFeatureSourceByCollection(sc);

				if (fs != null) {
					Rule[] polygonrules = new Rule[2];
					polygonrules[0] = StyleUtil.getPointTextRule(0.0, 0, null,
							"0x000000", "0", "0x000000", "1", "4", "0x000000",
							"10", "0x000000", "value");
					polygonrules[1] = StyleUtil.getTextRule(0.5, 1.5, null,
							"0x000000", "10", "sname", "0", "Normal");
					Style pointstyle = StyleUtil.getStyleByrules(polygonrules);
					pointlayer = new FeatureLayer(fs, pointstyle);
				}
				try {
					if (fs != null) {
						fs.getSchema();
						...
						System.err
								.println("getStationPointLayer 方法出错! fs==null ");
						return null;
					}
				} catch (Exception e) {
					System.err
							.println("getStationPointLayer 方法出错! fs.getSchema()==null ");
					return null;
				}
				return pointlayer;
			...
			e.printStackTrace();
			System.err.println("getStationPointLayer 方法出错!");
			return null;
		}
		return null;
